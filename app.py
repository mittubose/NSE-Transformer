...
def main():
    ...
    st.write(f"DEBUG: seq_length is {seq_length}") # Log for debugging
    
    # Model parameters and global settings
    # seq_length = 30 # Sequence length for transformer input
    # Define features globally as they are used across multiple functions and tabs
    # features = ['Open', 'High', 'Low', 'Close', 'Volume', 
    #             'RSI', 'MACD', 'MACD_signal', 'SMA_20', 'SMA_50',
    #             'BB_high', 'BB_low', 'BB_mid', 'EMA12', 'EMA26']
    ...
    # Timeframe filter (moved from sidebar to main app flow, before data fetching)
    timeframe_options = {
        "1 Day": 1, # Not strictly 1 day, but the last available day
        "1 Week": 7,
        "1 Month": 30,
        "3 Months": 90,
        "6 Months": 180,
    ...
    selected_timeframe = st.sidebar.selectbox("Select Timeframe", list(timeframe_options.keys()), index=6) # Default to 1 Year

    end_date = datetime.now()
    if selected_timeframe == "YTD":
    ...
    # Load data (now uses date_range determined by the main selector)
    try:
        ...
        # Fetch stock info (kept here)
        stock_info = ticker.info

        # Prepare data (kept here)
        ...
        with tab4:
            ...
            with st.spinner("Generating predictions..."):
                ...
                # Prepare sequences (now we are sure df has enough length)
                ...
                if len(X) == 0:
                    ...
                    # Also ensure the rest of the prediction tab content is not shown if no predictions can be made
                    st.info("Cannot generate predictions due to insufficient data.")
                    return # Exit the tab processing
                ...
                model.eval() # Set model back to eval mode after MCD
                ...
                for step in range(max_future_steps):
                    ...
                    next_input_row_values[features.index('Close')] = mean_pred_step
                    ...
                    for col in indicator_cols:
                        ...
                        for _ in range(num_monte_carlo_runs):
                            with torch.no_grad():
                                ...
                                perturbed_predictions.append(pred.item())
                        model.eval()
